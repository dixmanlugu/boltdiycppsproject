...
type Worker = { WorkerID: string; WorkerDOB: string | null };
...
function ageOn(dateISO: string, dobISO?: string | null): number | null {
...
function bucket(age: number | null): string {
...
const AgeGroupAccidentTypeReport: React.FC<Props> = ({
      ...
      try {
        ...
        if (workerIds.length) {
          const { data: w, error: wErr } = await supabase
            ...
            .select("WorkerID, WorkerDOB")
            .in("WorkerID", workerIds);
          if (wErr) throw wErr;
          for (const r of (w ?? []) as Worker[]) dobMap.set(r.WorkerID, r.WorkerDOB ?? null);
        }

        // Aggregate
        const groups = new Map<string, { Injury: number; Death: number; total: number }>();
        for (const r of incidents) {
          const dob = dobMap.get(r.WorkerID) ?? null;
          const theAge = ageOn(r.FirstSubmissionDate, dob);
          const g = bucket(theAge);
          const ex = groups.get(g) ?? { Injury: 0, Death: 0, total: 0 };
          if (String(r.IncidentType) === "Death") ex.Death += 1;
          else ex.Injury += 1; // default classify as Injury
          ex.total += 1;
          groups.set(g, ex);
        }

        const ordered = ["<20", "20–29", "30–39", "40–49", "50–59", "60+", "Unknown"].filter(g =>
          groups.has(g)
        );

        const table: AgeRow[] = ordered.map(g => ({
          group: g,
          Injury: groups.get(g)!.Injury,
          Death: groups.get(g)!.Death,
          total: groups.get(g)!.total,
        }));
  ...
  const exportCSV = () => {
    ...
    const header = "Age Group,Injury,Death,Total\n";
    const body = rows.map(r => `${r.group},${r.Injury},${r.Death},${r.total}`).join("\n");
  ...
  const exportPDF = async () => {
    ...
    // Render the visible content to canvas
    ...
    // Headings (mimic PHP header block)
  ...
  return (
    <div className="space-y-4">
      {/* Header line (internal controls only if standalone) */}
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={rows} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <Tooltip />
                <Legend />
                {/* Stacked per age group: Injury + Death */}
            ...
            {loading ? (
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-primary text-white">
                  <tr>
                    <th className="px-4 py-2 text-left text-xs font-semibold uppercase tracking-wider">Age Group</th>
...
export default AgeGroupAccidentTypeReport;
