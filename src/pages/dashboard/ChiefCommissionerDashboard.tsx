...
import ListApprovedClaimsForChiefCommissionerReview from '../../components/forms/ListApprovedClaimsForChiefCommissionerReview';
...
const ChiefCommissionerDashboard: React.FC = () => {
  ...
  const fetchCounts = async () => {
    try {
      ...
      const { count: pendingCount, error: pendingError } = await supabase
        .from('chief_commissioner_pending_view')
...
useEffect(() => {
  ...
  return (
    <div className="py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">Chief Commissioner Dashboard</h1>
        <p className="text-gray-600">Welcome back, {profile?.full_name || 'Chief Commissioner'}</p>
  ...
  <p className="text-gray-500 text-sm -mt-1">User ID: {userStaffID}</p>
              ...
              <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {index % 2 === 0 ? 'Injury Claim' : 'Death Claim'} - {['John Smith', 'Mary Johnson', 'Robert Brown', 'Sarah Davis'][index]}
                  </p>
                  <p className="text-sm text-gray-500">Submitted: {new Date(2023, 4, 15 - index).toLocaleDateString()}</p>
                </div>
                <div className="flex items-center space-x-2">
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                    ...
                    {index === 0 ? 'Pending Review' : 
                     index === 1 ? 'Documentation Required' : 
                     'Under Review'}
                  </span>
                  <button 
                    className="btn btn-primary text-sm"
                    onClick={() => setShowPendingClaimsList(true)}
                  >
                    Review
                  </button>
                </div>
          ...
          <div className="space-y-4">
            {[1, 2, 3, 4].map((_, index) => (
              <div key={index} className="p-4 bg-gray-50 rounded-lg">
                ...
                <p className="text-sm text-gray-500">Decision made on {new Date().toLocaleDateString()}</p>
                <button 
                  className="mt-2 text-sm text-primary hover:text-primary-dark font-medium"
                  onClick={() => setShowApprovedClaimsList(true)}
                >
                  View Details
                </button>
              </div>
      ...
      {showApprovedClaimsList && (
        <ListApprovedClaimsForChiefCommissionerReview 
...
export default ChiefCommissionerDashboard;
