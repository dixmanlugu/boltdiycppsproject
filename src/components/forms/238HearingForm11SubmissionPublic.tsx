...
const Form238HearingForm11SubmissionPublic: React.FC<Form238Props> = ({ irn, onClose }) => {
  ...
  useEffect(() => {
    ...
    const fetchFormData = async () => {
      try {
        ...
        if (!form1112Data) throw new Error('Form 1112 data not found for this IRN');
        ...
        if (!workerData) throw new Error('Worker personal details not found');

        setFormData({ ...form1112Data, ...workerData });
      } catch (err: any) {
...
export default Form238HearingForm11SubmissionPublic;
