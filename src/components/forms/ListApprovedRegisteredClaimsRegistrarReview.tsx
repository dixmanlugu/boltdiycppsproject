...
interface ListApprovedRegisteredClaimsRegistrarReviewProps {
  onClose: () => void;
...
const ListApprovedRegisteredClaimsRegistrarReview: React.FC<ListApprovedRegisteredClaimsRegistrarReviewProps> = ({ 
  onClose,
  onSelectIRN 
}) => {
  ...
  const fetchClaimsList = async () => {
    try {
      ...
      // Build the main query using the view
      let query = supabase
        .from('registrar_approved_view')
        .select('*')
  ...
  const handleView = (irn: string, rrid: string, incidentType: string) => {
    if (onSelectIRN) {
      ...
      switch (incidentType) {
        case 'Injury':
          url = '/dashboard/registrar/injury-approved';
          break;
        case 'Death':
          url = '/dashboard/registrar/death-approved';
          break;
      }
      
      if (url) {
        window.location.href = `${url}?IRN=${irn}&RRID=${rrid}`;
      }
    }
  };
                  ...
                  {claimsList.map((claim, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <button
                          onClick={() => handleView(claim.IRN, claim.RRID, claim.IncidentType)}
...
export default ListApprovedRegisteredClaimsRegistrarReview;
