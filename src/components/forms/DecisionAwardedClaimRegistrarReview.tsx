import React, { useState, useEffect } from 'react';
    ...
    const DecisionAwardedClaimRegistrarReview: React.FC = () => {
        ...
        if (router.query?.claim_id) {
          const fetchFormData = async () => {
            try {
              ...
              // Check for duplicate claim ID in CPOR table
              ...
              if (countError) {
                toast.error('Error checking claim ID status');
                return;
              }

              if (count > 0) {
                setIsDuplicate(true);
                toast.error('Claim ID already exists in CPOR table');
              }
            } catch (error) {
              toast.error('Error loading claim data');
            }
      ...
      const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (isDuplicate) {
          toast.error('Cannot process duplicate claim ID');
          return;
        }
        ...
        try {
          // Update claimsawardedregistrarreview table
          ...
          // Insert into claimsawardedpaymentsectionreview table if Approved
      ...
      return (
        <div className="bg-surface p-8 rounded-lg shadow-md max-w-2xl mx-auto">
          <h1 className="text-3xl font-bold mb-6 text-primary">Awarded Claim Decision Review</h1>
          
          {isDuplicate && (
            <div className="mb-6 p-4 bg-red-100 text-red-700 rounded-md">
              <p className="font-medium">⚠️ Warning: This claim ID already exists in the CPOR table</p>
            </div>
          )}
    ...
    export default DecisionAwardedClaimRegistrarReview;
