...
const SearchForm4: React.FC<SearchForm4Props> = ({
    ...
    try {
      ...
      if (displayCRN || workerID) {
        ...
        if (form1112Data && form1112Data.length > 0) {
          ...
          const form4IRNs = new Set(form4Data?.map((f) => f.IRN) || []);

          results = form1112Data.map((item) => {
            const worker = workerData?.find((w) => w.WorkerID === item.WorkerID);
            const hasForm4 = form4IRNs.has(item.IRN);

            let actionType: 'Proceed' | 'Edit' | 'View';
            let actionColor: string;

            if (formType === 'new') {
              actionType = hasForm4 ? 'View' : 'Proceed';
              actionColor = hasForm4 ? 'text-blue-600' : 'text-green-600';
            } else if (formType === 'edit') {
              actionType = hasForm4 ? 'Edit' : 'Proceed';
              actionColor = hasForm4 ? 'text-blue-600' : 'text-green-600';
            } else {
              actionType = hasForm4 ? 'View' : 'Proceed';
              actionColor = hasForm4 ? 'text-red-800' : 'text-green-600';
            }
  ...
  const handleAction = (result: SearchResult) => {
    ...
    if (result.actionType === 'Proceed' || (formType === 'new' && !result.hasForm4)) {
      setShowNewForm4(true);
    } else if (result.actionType === 'Edit' && formType === 'edit') {
      if (result.hasForm4) setShowEditForm4(true);
    } else if (result.actionType === 'View' || formType === 'view') {
      if (result.hasForm4) setShowViewForm4(true);
    }
  };
  ...
  // New Form 4
  if (showNewForm4 && selectedIRN != null) {
  ...
  // View Form 4
  if (showViewForm4 && selectedIRN != null) {
    return (
      <ViewForm4
  ...
  // Edit Form 4
  if (showEditForm4 && selectedIRN != null) {
    return (
      <EditForm4
        workerIRN={selectedIRN}
        onClose={() => {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 border-b sticky top-0 bg-white">
          <h2 className="text-xl font-semibold text-gray-900">
            Search Workers - Form 4 ({formType === 'new' ? 'New' : formType === 'view' ? 'View' : 'Edit'})
          </h2>
        ...
        <div className="p-6">
                          ...
                          <tr key={`${result.IRN}-${index}`} className="hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                              <span
                                className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                                  result.hasForm4 ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'
                                }`}
                              >
                                {result.hasForm4 ? 'Exists' : 'Not Created'}
                              </span>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <button
                                ...
                                disabled={formType === 'edit' && !result.hasForm4}
                              >
                                {result.actionType}
                              </button>
                            </td>
            ...
            {showNotFound && (
              <div className="text-center py-8">
                <p className="text-gray-600 mb-4">
                  No death cases found matching your search criteria.
                </p>
                <p className="text-sm text-gray-500">
                  Try adjusting your search terms or check if the worker has a death incident registered.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="mt-6 p-4 bg-gray-50 rounded-lg">
            ...
            <p className="text-sm text-gray-600">
              {formType === 'new' && 'Search for death cases to create new Form 4 applications or view existing ones.'}
              {formType === 'edit' && 'Search for death cases with existing Form 4 to edit them.'}
              {formType === 'view' && 'Search for death cases with existing Form 4 to view them.'}
            </p>
            <div className="mt-2 flex flex-wrap gap-4 text-xs">
              <div className="flex items-center">
                <div className="w-3 h-3 bg-green-100 border border-green-300 rounded mr-1"></div>
                <span className="text-green-600">Proceed - No Form 4 exists</span>
              </div>
              <div className="flex items-center">
                <div className="w-3 h-3 bg-blue-100 border border-blue-300 rounded mr-1"></div>
                <span className="text-blue-600">Edit/View - Form 4 exists</span>
              </div>
              {formType === 'view' && (
                <div className="flex items-center">
                  <div className="w-3 h-3 bg-red-100 border border-red-300 rounded mr-1"></div>
                  <span className="text-red-800">View - Form 4 exists</span>
                </div>
              )}
            </div>
          </div>
