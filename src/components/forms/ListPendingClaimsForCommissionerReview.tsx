...
interface ListPendingClaimsForCommissionerReviewProps {
  onClose: () => void;
...
const ListPendingClaimsForCommissionerReview: React.FC<ListPendingClaimsForCommissionerReviewProps> = ({ 
  onClose,
  onSelectIRN 
}) => {
  ...
  const handleProceed = (irn: string, incidentType: string) => {
    if (onSelectIRN) {
      ...
      switch (incidentType) {
        case 'Injury':
          url = '/dashboard/commissioner/injury-review';
          break;
        case 'Death':
          url = '/dashboard/commissioner/death-review';
          break;
      }
...
export default ListPendingClaimsForCommissionerReview;
