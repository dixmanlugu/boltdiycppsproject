...
interface ListPendingAwardedClaimsForRegistrarReviewProps {
  onClose: () => void;
...
const ListPendingAwardedClaimsForRegistrarReview: React.FC<ListPendingAwardedClaimsForRegistrarReviewProps> = ({ 
  onClose,
  onSelectIRN 
}) => {
...
const [activeReview, setActiveReview] = useState<'injury' | 'death' | null>(null);
  ...
  const fetchClaimsList = async () => {
    try {
      ...
      let countQuery = supabase
        .from('pending_awarded_claims_registrar_view')
  ...
  const handleProceed = (irn: string, incidentType: string) => {
    if (onSelectIRN) {
      ...
      switch (incidentType) {
        case 'Injury':
          url = '/dashboard/awarded-claims/injury-review';
          break;
        case 'Death':
          url = '/dashboard/awarded-claims/death-review';
          break;
      }
        ...
        <div className="p-6">
          ...
          {loading ? (
            ...
            <div className="text-center py-8">
              <p className="text-gray-600">No Awarded Claims Pending to Display</p>
            </div>
          )}
...
export default ListPendingAwardedClaimsForRegistrarReview;
