...
/**
 * ViewForm12.tsx
 * Read-only, printable "View mode" for Form 12 (Death)
 * - Resolves WorkerID & DisplayIRN from workerirn when needed
 * - Only runs WorkerID-based queries after WorkerID has been resolved
 */
...
export interface ViewForm12Props {
  irn?: number | string | null;   // optional, can resolve via workerId
  workerId?: string | null;       // optional, can resolve via workerirn using IRN
  onClose: () => void;
}
...
interface Form12Data {
  ...
  NatureExtentInjury: string; // left as-is (DB naming shared)
  InjuryCause: string;        // left as-is (DB naming shared)
  HandInjury: boolean;        // obsolete for Death but kept to map snapshot columns safely
  InjuryMachinery: boolean;   // same as above
  MachineType: string;
  ...
  // Common optional keys we populate from attachments
  SEC43?: string;
  SS?: string;
  WS?: string;
  PTA?: string;
  ...
  // Form12-specific keys
  DC?: string;   // Death Certificate
  PMR?: string;  // Post Mortem report
  DD?: string;   // Dependency declaration
  FER?: string;  // Funeral expenses receipts
...
// Component
// -----------------------------
const ViewForm12: React.FC<ViewForm12Props> = ({ irn, workerId: workerIdProp, onClose }) => {
  ...
  // Map normalized AttachmentType -> our keys
  const attachmentTypeToKey: Record<string, string> = useMemo(() => {
    const map: Record<string, string> = {
      ...
      // sometimes uploaded under these common names as well:
  ...
  // -----------------------------
  // 1) Resolve header (WorkerID, DisplayIRN, IRN)
  // -----------------------------
  useEffect(() => {
    let alive = true;
    (async () => {
      try {
        ...
        if (irnNum != null) {
          // We have IRN. Always fetch DisplayIRN; fetch WorkerID if missing.
          ...
          // No IRN provided: get latest Death IRN for this worker
          const { data, error } = await supabase
            .from("form1112master")
    ...
    (async () => {
      ...
      try {
        ...
        if (resolvedIRN != null) {
          ...
          // fallback: latest Death for worker
          const res = await supabase
            .from("form1112master")
            .select("*")
            .eq("WorkerID", workerId)
  ...
  const fetchAndHydrateAttachments = async (irnNum: number) => {
    try {
      ...
      const newPaths: Partial<Form12Data> = {};
      const previewUpdates: Record<string, string> = {};

      for (const r of rows || []) {
        const type = norm((r as any).AttachmentType);
        const key = attachmentTypeToKey[type];
  ...
  const HeaderTitle = () => (
    <>Form 12 — {displayIRN ?? (editingIRN ? `IRN ${editingIRN}` : "")}</>
  );
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-5xl max-h-[90vh] overflow-y-auto flex flex-col">
        ...
        {!headerLoading && (
          <div className="p-5 space-y-8">
            ...
            {/* Death Case Details */}
            <section className="space-y-4">
              <h3 className="text-lg font-semibold">Death Case Details</h3>
            ...
            <section className="space-y-4">
              ...
              <div>
                ...
                <div className="text-sm">{s(formData.InsuranceCompanyLandLine)}</div>
              </div>
            </section>

            {/* Form 12 Scan */}
            <section className="space-y-3">
              <h3 className="text-lg font-semibold">Form 12 Scan</h3>
            ...
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  ...
                  { key: "FER", label: "Funeral expenses receipts" },
                  { key: "PIR", label: "Police incident report" },
                  { key: "F18", label: "Form 18 Scan" },
                  // sometimes present (extra/optional uploads)
