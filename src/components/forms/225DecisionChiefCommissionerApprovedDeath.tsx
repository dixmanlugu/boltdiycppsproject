...
const Decision225ChiefCommissionerApprovedDeath: React.FC<Props> = ({
  ...
  // Generate the FINAL (non-preview) Consent of Award (Death) with stamp & signature
  const handleDownloadFinalPdf = async () => {
    ...
    // const isCommissioner = myStaffId === 2812; // reserved if you want role-specific assets
  ...
  const handlePrintForm6 = async () => {
    ...
    try {
      ...
      console.error(e);
      alert('Failed to open Form 6.');
    }
  };

  // Print Form 18 (jsPDF util)
  const handlePrintForm18 = async () => {
    ...
    try {
      ...
      const fn =
        (mod as any).default ??
        (mod as any).printForm18CPO ??
        (mod as any).generateForm18;
      if (typeof fn === 'function') {
        await fn(resolvedIRN);
      } else {
